 ...
 */
export function getResolvedTokenKey(ref, ctx) {
    const canonical = canonicalizeRefPath(ref);
    const normalized = normalizePathKey(canonical);
    const hasKey = (key) => ctx.valueMap.has(key) || ctx.refMap.has(key);
    if (ctx.collisionKeys.has(normalized))
        return null;
    if (hasKey(canonical))
        return canonical;
 ...
 */
export function getResolvedTokenKeyFromParts(canonical, normalized, ctx) {
    const hasKey = (key) => ctx.valueMap.has(key) || ctx.refMap.has(key);
    if (ctx.collisionKeys.has(normalized))
        return null;
    if (hasKey(canonical))
 ...
 */
export function collectRefsFromValue(value, refs, idToTokenKey) {
    ...
    if (isPlainObject(value)) {
        for (const key of Object.keys(value)) {
            if (!key.startsWith('$')) {
                collectRefsFromValue(value[key], refs, idToTokenKey);
            }
        }
    }
}
 ...
 */
export function buildCycleStatus(ctx) {
    ...
    const dfs = (node) => {
        ...
        if (state === 1)
            return true;
        if (state === 2)
            return leadsToCycle.get(node) ?? false;
        color.set(node, 1);
        let hitCycle = false;
