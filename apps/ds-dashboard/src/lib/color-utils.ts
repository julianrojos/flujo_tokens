...
export function computeContrastRatio(backgroundCssColor: string, foregroundCssColor: string): number {
  ...
  if (!backgroundRaw || !foregroundRaw) {
    const which = !backgroundRaw ? `background="${backgroundCssColor}"` : `foreground="${foregroundCssColor}"`;
    throw new Error(`Unable to compute contrast ratio: invalid CSS color input (${which}).`);
  }
