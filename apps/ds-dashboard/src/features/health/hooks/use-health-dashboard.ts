...
export function useHealthDashboard() {
  ...
  const [brokenAliasSort, toggleBrokenAliasSort] = useSortState<"token" | "alias" | "reason">({
    field: "token",
    dir: "asc",
  });
  ...
  const brokenAliases = useMemo(() => {
    ...
    return items.slice().sort((a, b) => {
      ...
      if (brokenAliasSort.field === "alias") {
        return compareStrings(a.aliasCssVar, b.aliasCssVar, brokenAliasSort.dir);
      }
  ...
  const handleIssueViewClick = useCallback((event: MouseEvent<HTMLAnchorElement>, to: string) => {
