...
/**
 * Integration tests for useComponentAdoption hook
 * 
 * Note: Full hook testing requires @testing-library/react for renderHook.
 * These tests would verify:
 * - Initial state: { loading: true, error: null, reports: [] }
 * - After fetch success: { loading: false, error: null, reports: [...] }
 * - After fetch failure: { loading: false, error: "message", reports: [] }
 * - Cleanup on unmount (local active flag = false)
 * - Refetch when slug or dsFileKey changes
 * 
 * Example (pending RTL setup):
 * 
 * describe("useComponentAdoption (hook integration)", () => {
 *   it("starts with loading: true and transitions to loading: false on success", async () => {
 *     const { result } = renderHook(() => useComponentAdoption({ slug: "boton", allItems: BASE_ITEMS }), {
 *       wrapper: ({ children }) => (
 *         <DesignSystemProvider activeSystem="sys-01">
 *           {children}
 ...
 *     assert.equal(result.current.error, null);
 *     assert.ok(Array.isArray(result.current.reports));
 *   });
 * 
 *   it("sets error state when fetch fails", async () => {
 *     // Mock fetchReportByComponent to throw
 *     // ...
 *     await waitFor(() => assert.ok(result.current.error !== null));
 *   });
 * 
 *   it("returns early when slug is undefined", async () => {
 *     const { result } = renderHook(() => useComponentAdoption({ slug: undefined, allItems: BASE_ITEMS }));
 *     await waitFor(() => assert.equal(result.current.loading, false));
 *     assert.deepEqual(result.current.reports, []);
 *   });
 * 
 *   it("returns early when dsFileKey is null", async () => {
 *     // Mock useDsFileKey to return null
 *     // ...
 *     await waitFor(() => assert.equal(result.current.loading, false));
 *     assert.deepEqual(result.current.reports, []);
 *   });
