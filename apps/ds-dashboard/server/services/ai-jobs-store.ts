...
export class AiJobsStore {
  ...
  enqueue(input: AiJobInput, idempotencyKeyOverride?: string): AiJobState {
    ...
    if (this.jobs.size >= MAX_JOBS) {
      const error = new Error('Job queue is at capacity') as Error & {
        code: string;
        retryable: boolean;
      };
    ...
    const effectiveKey =
      idempotencyKeyOverride ?? this.computeIdempotencyKey(input);
  ...
  setPipelineStage(
    jobId: string,
    stage: 'extracting' | 'patching' | 'validating',
  ): void {
    const job = this.jobs.get(jobId);
    if (!job) {
      return;
  ...
  fail(jobId: string, error: string, code: string, retryable: boolean): void {
    ...
    if (job.status === 'cancelled') {
      this.pushEvent(job.id, 'job.fail_attempted_after_cancel', {
        code,
        error,
        retryable,
      });
...
export function getAiJobsStore(): AiJobsStore {
  if (!storeInstance) {
    throw new Error(
      '[AiJobsStore] Store not initialized. Call initializeAiJobsStore() before use.',
    );
  }
