...
test("component catalog leaves missing visual proof dimensions unset", async () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'catalog-route-handler-'));
  const proofImageRel =
    'design-systems/pfb-design-system/docs/_generated/visual-proofs/images/button.png';
  const proofImageAbs = path.join(repoRoot, proofImageRel);
  fs.mkdirSync(path.dirname(proofImageAbs), { recursive: true });
  fs.writeFileSync(proofImageAbs, Buffer.from('fixture image', 'utf8'));
