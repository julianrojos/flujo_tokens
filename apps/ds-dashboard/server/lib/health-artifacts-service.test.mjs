...
test("health-artifacts-service: buildEmptyTokenHealthReport keeps bootstrap shape", () => {
...
test("health-artifacts-service: buildEmptyComponentsHealthReport keeps bootstrap shape", () => {
...
test("health-artifacts-service: normalizeHealthHistoryRange defaults unknown values", () => {
...
test("health-artifacts-service: normalizeHealthHistoryPayload sanitizes snapshots", () => {
...
test("health-artifacts-service: filterSnapshotsByRange keeps only recent snapshots", () => {
  const now = Date.now();
