/**
 * Utility for ${VAR} env-reference handling.
 *
 * - normalizeEnvRef: normalize user/config input to canonical "${VAR}" for storage.
 * - resolveEnvRef: resolve "${VAR}" against process.env at runtime.
 */

/**
 * Normalize to canonical "${VAR_NAME}" shape for config storage.
 * Accepts "${VAR}", "$VAR", "VAR" (plain identifier), or literal strings.
 */
...
/**
 * Resolve "${VAR_NAME}" against process.env.
