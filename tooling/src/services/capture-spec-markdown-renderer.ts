...
export function renderAnatomyMarkdown(anatomy: SpecAnatomyItem[] | undefined | null): string {
  ...
  for (let i = 0; i < anatomy.length; i++) {
    ...
    if (part.dimensions) {
      ...
      if (part.width) attrs.push(`Width ${part.width}`);
      if (part.height) attrs.push(`Height ${part.height}`);
    }
    if (part.description) attrs.push(`Description ${part.description}`);
...
export function renderPropertiesTable(properties: SpecProperty[] | undefined | null): string {
  ...
  for (const prop of properties) {
    ...
    const notes = prop.narrative_notes ?? prop.notes ?? '';
    rows.push(
